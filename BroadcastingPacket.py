...
class BroadcastingPacket:
    def __init__(self, SourceID, PacketPara):
        ...
        self.minisensi = BroadcastingPacket.GetReceiveSensitivity(self.sf,self.bw)
        self.miniSNR = BroadcastingPacket.GetMiniSNR(self.sf)
        self.rectime = BroadcastingPacket.airtime(self.sf,self.cr,self.PS,self.bw)
        self.tx_energy = BroadcastingPacket.calculate_energy(self.tp,self.rectime)
    ...
    def calculate_energy(tp, airtime):
        return BroadcastingPacket.dbm_to_watt(tp) * airtime
